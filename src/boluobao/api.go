...
func GetBookDetailedById(bookId string) *sfacg_structs.BookInfo {
	return req.Get(fmt.Sprintf(req.SFBookDetailedById, bookId), &sfacg_structs.BookInfo{}).(*sfacg_structs.BookInfo)
}

func GetAccountDetailedByApi() *sfacg_structs.Account {
	return req.Get(req.SFAccountDetailedByApi, &sfacg_structs.Account{}).(*sfacg_structs.Account)
}

func GetCatalogue(NovelID string) *sfacg_structs.Catalogue {
	return req.Get(fmt.Sprintf(req.SFCatalogueDetailedById, NovelID), &sfacg_structs.Catalogue{}).(*sfacg_structs.Catalogue)
}

func GetContentDetailedByCid(cid string) (*sfacg_structs.Content, bool) {
	result := req.Get(fmt.Sprintf(req.SFContentDetailedByCid, cid), &sfacg_structs.Content{}).(*sfacg_structs.Content)
...
func GetSearchDetailedByKeyword(keyword string, page int) *sfacg_structs.Search {
	return req.Get(fmt.Sprintf(req.SFSearch, url_.QueryEscape(keyword), page), &sfacg_structs.Search{}).(*sfacg_structs.Search)
...
func PostLoginByAccount(username, password string) *sfacg_structs.Login {
	...
	response, Cookie := req.LoginSession(req.SET_URL(req.SFLogin), data)
