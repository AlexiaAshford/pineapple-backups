...
func GetCatalogue(BookData Books) {
	response, TestInfo := boluobao.Get_catalogue_detailed_by_id(BookData.NovelID), TestInit(BookData.NovelName)
	...
	for _, data := range response.Data.VolumeList {
		...
		for _, Chapter := range data.ChapterList {
			...
			if !TestInfo.In(ChapId) {
				GetContent(len(data.ChapterList), BookData.NovelName, ChapId, TestInfo)
			}
		}
	}
...
func GetContent(ChapterLength int, NovelName, cid string, TestInfo *TestChapterConfig) {
